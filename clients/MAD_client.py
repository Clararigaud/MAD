...
class MAD_client:
	...
	def sendFileToProject(self, project, file):
		...
		r = requests.post('http://%s:%s/mad/post'%(self.server_address,str(self.server_port)), data = {'projectid':project}, files= {'upload' : open(file, 'rb')})

	def getProjects(self):
		r = requests.get('http://%s:%s/mad/getdata'%(self.server_address,str(self.server_port)), data = {'request':'projects'})
		return r.json()